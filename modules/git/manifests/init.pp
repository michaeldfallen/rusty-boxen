...
class git (
  ...
  zsh::alias { 'prune-branches': cmd => "git fetch --prune && git branch -r | awk \\\'{print \$1}\\\' | egrep -v -f /dev/fd/0 <(git branch -vv | grep origin) | awk \\\'{print \$1}\\\' | (xargs git branch -d || true)" }
}
